function [matriztrans,normal_puntos]=maxareafun2(vertices_borde,vertices,normales,borde_hueco,dibuja_maxarea)
%--------------------------------------
%Prueba maximización de área
%-------------------------------------
global p;
p_ini=vertices_borde;
p=vertices_borde;
...
matriztrans(1:3,3)=matriztrans(1:3,3)./norm(matriztrans(1:3,3));
matriztrans=0;
normal_puntos=x(:,3)';
normal_puntos=sign(dot(mean(normales(borde_hueco,:)),normal_puntos)).*normal_puntos;
save datos_proy.mat v_ini p_ini cdg borde_hueco normales;
end
