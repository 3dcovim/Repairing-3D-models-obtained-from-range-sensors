function [matriztrans,normal_puntos]=maxareafun3(vertices_borde,vertices_borde_ext,vertices,normales,borde_hueco,dibuja_maxarea)
%--------------------------------------
...
p_ini_ext=vertices_borde_ext;
p_ext=vertices_borde_ext;
...
[x,fval] = fmincon(@funcion_objetivo2,x0,[],[],[],[],[],[],...
    @conf,options);
x(:,1)=x(:,1)/norm(x(:,1));
x(:,2)=x(:,2)/norm(x(:,2));
x(:,3)=cross(x(:,1),x(:,2));
