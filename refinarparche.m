...
F2=[];
for indica=1:size(centroid,1)
    V=[V;centroid(indica,:)];
    new_indice=size(V,1);
    F2=[F2;new_indice F(indica,1) F(indica,2)];
    F2=[F2;new_indice F(indica,2) F(indica,3)];
    F2=[F2;new_indice F(indica,3) F(indica,1)];
